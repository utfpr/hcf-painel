...
const CardTombo = ({ tombo, onClickViewTombo }) => {
    const {
        ...
        local_coleta: localColeta, data_coleta_ano: dataColetaAno,
    ...
    }, [dataColetaAno, dataColetaDia, dataColetaMes]);

    const image = useMemo(() => {
        if (fotoPrincipal) {
            return `${strings.urlImage}/crop/240/200/${fotoPrincipal}`;
        }
        return 'error';
    }, [fotoPrincipal]);
        ...
        >
            ...
            <div className={styles.content}>
                ...
                <Space>
                    <FaUserAlt />
                    {coletores?.length
                        ? coletores?.map(coletor => coletor.nome).join(', ') : 'Coletor não definido'}
                </Space>
            </div>
            <div>
                <Space>
            ...
            <div>
                <Space>
                    <HomeFilled />
                    {localColeta?.cidade?.nome || 'Local de coleta não definido'}
                </Space>
            </div>
        </Card>
    );
