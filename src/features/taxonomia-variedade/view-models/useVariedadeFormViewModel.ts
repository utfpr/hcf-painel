...
export interface VariedadeFormViewModel {
  form: ReturnType<typeof useForm<VariedadeFormValues>>
  ...
  openCreate: () => void
  openForEdit: (variedade: Variedade) => Promise<void>
  closeModal: () => void
...
export interface UseVariedadeFormViewModelOptions {
  onSuccess: () => void | Promise<void>
}

export function useVariedadeFormViewModel(options: UseVariedadeFormViewModelOptions): VariedadeFormViewModel {
  ...
  const form = useForm<VariedadeFormValues>({
    defaultValues: {
      nomeVariedade: '',
      reinoId: undefined,
      familiaId: undefined,
  ...
  const openForEdit = useCallback(
    async (variedade: Variedade) => {
      setModalTitle('Atualizar')
      setEditingId(variedade.id)
      const reinoId = variedade.especie?.genero?.familia?.reino?.id ?? null
      const familiaId = variedade.especie?.genero?.familia?.id ?? null
      const generoId = variedade.especie?.genero?.id ?? null
      const especieId = variedade.especie?.id ?? null
      const autorId = variedade.autor?.id
  ...
  const submitModal = useCallback(async () => {
    const nome = form.getValues('nomeVariedade')?.trim()
    ...
    if (!nome || !especieId) {
      notifyRef.current({
        type: 'warning',
        message: 'Falha',
        description: 'Informe o nome da nova variedade e da espécie.'
      })
      return
    }
