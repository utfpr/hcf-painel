    ...
    */
        ...
        axios.get('/specieslink', { params }).then(response => {
            if (response.status === 200) {
                if (response.data.result === 'failed') {
                    this.exibeNotificacao(this.props.t('common:error'), this.props.t('common:failure'), this.props.t('servicosSpeciesLink:erroAgendarAtualizacao'))
                } else {
                    if (params.periodicidade === 2) {
                        this.exibeNotificacao(this.props.t('common:success'), this.props.t('common:success'), this.mensagemSemanal(moment().isoWeekday()))
                    }
                    if (params.periodicidade === 3) {
                        this.exibeNotificacao(this.props.t('common:success'), this.props.t('common:success'), this.mensagemMensal())
                    }
                    if (params.periodicidade === 4) {
                        this.exibeNotificacao(this.props.t('common:success'), this.props.t('common:success'), this.mensagem2Mensal())
                    }
                }
            }
        })
    ...
    comparaInformacoesSpeciesLink = () => {
        ...
        axios.get('/specieslink', { params }).then(response => {
            if (response.status === 200) {
                if (response.data.result === 'failed') {
                    this.exibeNotificacao(this.props.t('common:error'), this.props.t('common:failure'), this.props.t('servicosSpeciesLink:erroProcessoEmExecucao'))
                } else {
                    this.exibeNotificacao(this.props.t('common:success'), this.props.t('common:success'), this.props.t('servicosSpeciesLink:sucessoAtualizacaoInicializada'))
                }
            }
        })
    }
