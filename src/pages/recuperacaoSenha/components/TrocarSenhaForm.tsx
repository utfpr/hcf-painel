...
export default function TrocarSenhaForm({ token, onSuccess }: Props) {
  ...
  const handleFinish = async (values: { password: string; confirm: string }) => {
    ...
    try {
      ...
      await axios.put(`${apiUrl}/usuarios/redefinir-senha`, {
        token,
      ...
      message.error(err.message || 'Falha ao redefinir senha. Tente novamente.')
    } finally {
      setLoading(false)
    }
  }
